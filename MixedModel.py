...
def mixed_model(u_prob_dict, b_prob_dict, test_df, genres):
    ...
    while greek_lambda >= 0:
        ...
        for i in range(len(test_df)):
            ...
            # print(f'Song # {i + 1}')
            # print(
            #     f"Max Probability Genre: {max_genre} with probability {max_value}")
        ...
        # print(
        #     f'Best TP Count: {best_tp_cnt} with Lambda: {best_lambda} with best res list: {best_res_df} \n')
        print(greek_lambda, curr_tp_cnt, best_tp_cnt, best_lambda)
        greek_lambda -= 0.01
    print(best_res_df)
    print(f'Lambda: {best_lambda}')
    confusion_matrix(best_res_df, genres)
    f1_score(best_res_df)
...
def f1_score(df):
    ...
    for cls in classes:
        ...
        tp = len(df[(df['Predicted'] == cls) & (df['Genre'] == cls)])
        # False positives: Predicted = cls but Actual != cls
        fp = len(df[(df['Predicted'] == cls) & (df['Genre'] != cls)])
        # False negatives: Predicted != cls but Actual = cls
        fn = len(df[(df['Predicted'] != cls) & (df['Genre'] == cls)])
