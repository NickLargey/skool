...
def read_filtered_images(file_path):
    ...
    with open(file_path, newline='') as file:
        ...
        for line in tsv_file:
            image = line[0].strip()
            files[image.split('.')[0]] = image
    return files


def reading_captions(file_path, files):
    dic_file_images = {}
    with open(file_path, newline='') as file:
        tsv_file = csv.reader(file, delimiter='\t', lineterminator='\n', quotechar='"')
        for line in tsv_file:
            ...
            if file_name not in files:
                continue
            caption = line[1]
            dic_file_images[files[file_name]] = caption
    return dic_file_images


files = read_filtered_images("mse_timath_filenames.tsv")
# print(len(lst_files))
dic_file_caption = reading_captions("../MSE.tsv", files)
...
with open("mse_clip_scores.tsv", "w", newline='') as file:
    ...
    for file_name in tqdm(dic_file_caption):
        ...
        img_emb = model.encode(Image.open("../MSEImages/" + file_name))
